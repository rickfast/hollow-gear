...
describe('Species System', () => {
  describe('SpeciesTraitsUtils', () => {
    describe('getSpeciesDisplayName', () => {
      it('should return correct display names for all species', () => {
        expect(SpeciesTraitsUtils.getSpeciesDisplayName('aqualoth')).toBe(
          'Aqualoth'
        );
        expect(SpeciesTraitsUtils.getSpeciesDisplayName('vulmir')).toBe(
          'Vulmir'
        );
        expect(SpeciesTraitsUtils.getSpeciesDisplayName('rendai')).toBe(
          'Rendai'
        );
        expect(SpeciesTraitsUtils.getSpeciesDisplayName('karnathi')).toBe(
          'Karnathi'
        );
        expect(SpeciesTraitsUtils.getSpeciesDisplayName('tharn')).toBe('Tharn');
        expect(SpeciesTraitsUtils.getSpeciesDisplayName('skellin')).toBe(
          'Skellin'
        );
        expect(SpeciesTraitsUtils.getSpeciesDisplayName('avenar')).toBe(
          'Avenar'
        );
      });
    });
    ...
    describe('getSpeciesAnimalType', () => {
      it('should return correct animal types for all species', () => {
        expect(SpeciesTraitsUtils.getSpeciesAnimalType('aqualoth')).toBe(
          'Axolotl'
        );
        expect(SpeciesTraitsUtils.getSpeciesAnimalType('vulmir')).toBe('Fox');
        expect(SpeciesTraitsUtils.getSpeciesAnimalType('rendai')).toBe(
          'Red Panda'
        );
        expect(SpeciesTraitsUtils.getSpeciesAnimalType('karnathi')).toBe(
          'Ibex'
        );
        expect(SpeciesTraitsUtils.getSpeciesAnimalType('tharn')).toBe('Elk');
        expect(SpeciesTraitsUtils.getSpeciesAnimalType('skellin')).toBe(
          'Gecko'
        );
    ...
    describe('hasSpecialAbility', () => {
      it('should correctly identify species special abilities', () => {
        expect(
          SpeciesTraitsUtils.hasSpecialAbility(AQUALOTH_TRAITS, 'amphibious')
        ).toBe(true);
        expect(
          SpeciesTraitsUtils.hasSpecialAbility(AQUALOTH_TRAITS, 'echo_mimicry')
        ).toBe(false);

        expect(
          SpeciesTraitsUtils.hasSpecialAbility(VULMIR_TRAITS, 'echo_mimicry')
        ).toBe(true);
        expect(
          SpeciesTraitsUtils.hasSpecialAbility(VULMIR_TRAITS, 'amphibious')
        ).toBe(false);
      });
    });
    ...
    describe('getSpecialAbility', () => {
      it('should return correct special ability by ID', () => {
        const ability = SpeciesTraitsUtils.getSpecialAbility(
          SKELLIN_TRAITS,
          'wall_skitter'
        );
  ...
  describe('Species Trait Application and Ability Score Modifications', () => {
    describe('Ability Score Increases', () => {
      it('should have correct ability score increases for Aqualoth', () => {
        ...
        const intIncrease = increases.find(
          inc => inc.ability === 'intelligence'
        );
      ...
      it('should have correct ability score increases for Tharn', () => {
        ...
        const conIncrease = increases.find(
  ...
  describe('Species Data Completeness and Consistency', () => {
    describe('SPECIES_TRAITS_REGISTRY', () => {
      ...
      it('should have consistent trait structure for all species', () => {
        Object.values(SPECIES_TRAITS_REGISTRY).forEach(traits => {
          ...
          expect(
            traits.abilityScoreIncrease.every(inc => inc.increase > 0)
          ).toBe(true);
    ...
    describe('SpeciesDataUtils', () => {
      ...
      it('should find species by ability increase', () => {
        const intSpecies =
          SpeciesDataUtils.getSpeciesByAbilityIncrease('intelligence');
        ...
        const strSpecies =
          SpeciesDataUtils.getSpeciesByAbilityIncrease('strength');
      ...
      it('should find species by special ability', () => {
        const amphibiousSpecies =
          SpeciesDataUtils.getSpeciesByAbility('amphibious');
        expect(amphibiousSpecies).toContain('aqualoth');
        expect(amphibiousSpecies).toHaveLength(1);

        const wallSkitterSpecies =
          SpeciesDataUtils.getSpeciesByAbility('wall_skitter');
    ...
    describe('Species Trait Validation', () => {
      ...
      it('should reject species without ability score increases', () => {
        ...
        if (!result.success) {
          expect(
            result.error.some(e => e.code === 'MISSING_ABILITY_INCREASES')
          ).toBe(true);
        }
      });
      ...
      it('should reject species with invalid walking speed', () => {
        ...
        expect(result.success).toBe(false);
        if (!result.success) {
          expect(result.error.some(e => e.code === 'INVALID_WALK_SPEED')).toBe(
            true
          );
        }
      });
      ...
      it('should reject species without Common language', () => {
        ...
        if (!result.success) {
          expect(
            result.error.some(e => e.code === 'MISSING_COMMON_LANGUAGE')
          ).toBe(true);
        }
      });
      ...
      it('should reject species with incomplete Etherborne traits', () => {
        ...
        if (!result.success) {
          expect(
            result.error.some(e => e.code === 'MISSING_ETHERBORNE_TRAITS')
          ).toBe(true);
        }
      });
    });
  ...
  describe('Integration and Edge Cases', () => {
    ...
    it('should handle species with usage-limited abilities', () => {
      const vulmirShadowStep = VULMIR_TRAITS.specialAbilities.find(
        a => a.id === 'shadow_step'
      );
      expect(vulmirShadowStep?.usage?.rechargeType).toBe('long');
      expect(vulmirShadowStep?.usage?.usesPerRecharge).toBe(1);

      const tharnLinked = THARN_TRAITS.specialAbilities.find(
        a => a.id === 'aether_linked'
      );
    ...
    it('should handle species with damage-dealing abilities', () => {
      const tharnCharge = THARN_TRAITS.specialAbilities.find(
        a => a.id === 'charge'
      );
      ...
      const rendaiJuryRig = RENDAI_TRAITS.specialAbilities.find(
        a => a.id === 'jury_rig'
      );
    ...
    it('should maintain consistency across all species data', () => {
      ...
      allSpecies.forEach(species => {
        ...
        const total = traits.abilityScoreIncrease.reduce(
          (sum, inc) => sum + inc.increase,
          0
        );
        expect(total).toBe(3);

        // One increase should be +2, one should be +1
        const increases = traits.abilityScoreIncrease
          .map(inc => inc.increase)
          .sort();
