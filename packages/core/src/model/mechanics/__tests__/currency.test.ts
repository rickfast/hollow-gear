...
describe("Currency System", () => {
  ...
  describe("convertFromTotalCogs", () => {
    it("should convert total cogs back to currency breakdown", () => {
      expect(convertFromTotalCogs(125)).toEqual({ cogs: 5, gears: 2, cores: 1 });
      ...
      expect(convertFromTotalCogs(200)).toEqual({ cogs: 0, gears: 0, cores: 2 });
      expect(convertFromTotalCogs(157)).toEqual({ cogs: 7, gears: 5, cores: 1 });
    });
  });
...
describe("Aether Cell System", () => {
  ...
  describe("findBestAetherCell", () => {
    ...
    it("should prefer better condition cells", () => {
      ...
      const damagedCell = { ...createAetherCell("standard"), charges: 5, condition: "damaged" as AetherCellCondition };
...
describe("Currency Constants", () => {
  ...
  it("should have condition effects for all conditions", () => {
    const conditions: AetherCellCondition[] = ["pristine", "good", "worn", "damaged", "depleted"];
    
    conditions.forEach(condition => {
      expect(CONDITION_EFFECTS[condition]).toBeDefined();
      expect(CONDITION_EFFECTS[condition].chargeEfficiency).toBeGreaterThanOrEqual(0);
      expect(CONDITION_EFFECTS[condition].malfunctionRisk).toBeGreaterThanOrEqual(0);
    });
  });
});