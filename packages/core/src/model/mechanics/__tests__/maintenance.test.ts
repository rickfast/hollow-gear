...
describe("Malfunction Risk System", () => {
  ...
  describe("createMalfunction", () => {
    ...
    it("should generate repair requirements", () => {
      ...
      const skillCheck = malfunction.repairRequirements.find(r => r.type === "skill_check");
      expect(skillCheck).toBeDefined();
      expect(skillCheck?.requirement).toBe("Tinker Tools");
      
      const timeReq = malfunction.repairRequirements.find(r => r.type === "time");
...
describe("Maintenance System", () => {
  describe("performMaintenance", () => {
    ...
    it("should cause malfunction on critical failure", () => {
      ...
      expect(result.notes).toContain("Maintenance error caused new malfunction!");
...
describe("Maintenance Constants", () => {
  it("should have malfunction severity effects", () => {
    const severities: MalfunctionSeverity[] = ["minor", "moderate", "major", "critical"];
    
    severities.forEach(severity => {
      expect(MALFUNCTION_SEVERITY_EFFECTS[severity]).toBeDefined();
      expect(MALFUNCTION_SEVERITY_EFFECTS[severity].repairDifficulty).toBeDefined();
      expect(MALFUNCTION_SEVERITY_EFFECTS[severity].performancePenalty).toBeGreaterThan(0);
      expect(MALFUNCTION_SEVERITY_EFFECTS[severity].riskOfSpread).toBeGreaterThanOrEqual(0);
    });
  });

  it("should have maintenance difficulty DCs", () => {
    const difficulties: MaintenanceDifficulty[] = ["trivial", "easy", "moderate", "hard", "very_hard"];
    ...
    expect(MAINTENANCE_DIFFICULTY_DC.trivial).toBeLessThan(MAINTENANCE_DIFFICULTY_DC.easy);
    expect(MAINTENANCE_DIFFICULTY_DC.easy).toBeLessThan(MAINTENANCE_DIFFICULTY_DC.moderate);
    expect(MAINTENANCE_DIFFICULTY_DC.moderate).toBeLessThan(MAINTENANCE_DIFFICULTY_DC.hard);
    expect(MAINTENANCE_DIFFICULTY_DC.hard).toBeLessThan(MAINTENANCE_DIFFICULTY_DC.very_hard);
  });
