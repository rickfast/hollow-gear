  ...
  describe('Arcanist Spellcasting (Aether Formulae)', () => {
    ...
    describe('ArcanistUtils calculations', () => {
      ...
      it('should calculate heat generation', () => {
        // Normal casting
        expect(
          ArcanistUtils.calculateHeatGeneration(mockFormula, 1, false, 1.8)
        ).toBe(1);
        expect(
          ArcanistUtils.calculateHeatGeneration(mockFormula, 2, false, 1.8)
        ).toBe(2); // +1 for higher level

        // Overclocked casting
        expect(
          ArcanistUtils.calculateHeatGeneration(mockFormula, 1, true, 1.8)
        ).toBe(1); // floor(1 * 1.8) = 1
        expect(
          ArcanistUtils.calculateHeatGeneration(mockFormula, 2, true, 1.8)
        ).toBe(3); // floor(2 * 1.8) = 3
      });
    ...
    describe('Arcanist spell casting', () => {
      it('should successfully cast spell when resources are sufficient', () => {
        const result = ArcanistUtils.castFormula(
          mockArcanistData,
          mockFormula,
          1,
          false
      ...
      it('should fail when spell level exceeds Equilibrium Tier', () => {
        ...
        expect(result.errors).toContain(
          'Spell level 5 exceeds Equilibrium Tier 3'
        );
      });
      ...
      it('should fail when would exceed maximum heat', () => {
        ...
        expect(result.success).toBe(false);
        expect(result.errors).toContain(
          'Would exceed maximum heat points (11/10)'
        );
      });
    });
    ...
    describe('Arcanist resource management', () => {
      ...
      it('should validate Arcanist data correctly', () => {
        const validResult =
          ArcanistUtils.validateArcanistData(mockArcanistData);
  ...
  describe('Templar Spellcasting (Resonance Charges)', () => {
    ...
    describe('TemplarUtils calculations', () => {
      ...
      it('should calculate faith feedback generation', () => {
        // Normal casting
        expect(TemplarUtils.calculateFaithFeedback(mockMiracle, 1, false)).toBe(
          1
        );
        expect(TemplarUtils.calculateFaithFeedback(mockMiracle, 2, false)).toBe(
          2
        ); // +1 for higher level

        // Overchanneled casting
        expect(TemplarUtils.calculateFaithFeedback(mockMiracle, 1, true)).toBe(
          2
        ); // 1 * 2
        expect(TemplarUtils.calculateFaithFeedback(mockMiracle, 2, true)).toBe(
          4
        ); // 2 * 2
      });

      it('should calculate harmony bonus', () => {
        const recentCastings: ResonanceType[] = [
          'restorative',
          'restorative',
          'divine',
        ];
        ...
        const bonus = TemplarUtils.calculateHarmonyBonus(
          5,
          'restorative',
          recentCastings
        );
        ...
        // Different type
        const bonus2 = TemplarUtils.calculateHarmonyBonus(
          5,
          'protective',
          recentCastings
        );
    ...
    describe('Templar spell casting', () => {
      ...
      it('should fail when would exceed maximum faith feedback', () => {
        ...
        expect(result.success).toBe(false);
        expect(result.errors).toContain(
          'Would exceed maximum faith feedback (16/15)'
        );
      });
    });
    ...
    describe('Templar resource management', () => {
      it('should restore Overchannel uses on rest', () => {
        ...
        expect(restored.overchannelUses).toBe(
          mockTemplarData.maxOverchannelUses
        );
      });
      ...
      it('should reduce faith feedback', () => {
        ...
        const overReduced = TemplarUtils.reduceFaithFeedback(
          mockTemplarData,
          15
        );
  ...
  describe('Heat Generation and Feedback Systems', () => {
    ...
    describe('Heat point management', () => {
      ...
      it('should calculate heat dissipation', () => {
        expect(HeatFeedbackUtils.calculateHeatDissipation('short', 5, 2)).toBe(
          2
        );
        expect(HeatFeedbackUtils.calculateHeatDissipation('long', 5, 2)).toBe(
          5
        ); // All heat
        expect(HeatFeedbackUtils.calculateHeatDissipation('short', 1, 2)).toBe(
          1
        ); // Can't exceed current
      });

      it('should add heat points and calculate feedback', () => {
        const updated = HeatFeedbackUtils.addHeatPoints(
    ...
    describe('Feedback effects', () => {
      ...
      it('should generate appropriate feedback effects at different levels', () => {
        ...
        const effects25 = HeatFeedbackUtils.calculateFeedbackEffects(
          25,
          'arcanist',
          []
        );
        expect(effects25.some(e => e.type === 'spell_attack_penalty')).toBe(
          true
        );
        expect(effects25.some(e => e.type === 'concentration_penalty')).toBe(
          true
        );

        // 50% feedback - additional penalties
        const effects50 = HeatFeedbackUtils.calculateFeedbackEffects(
          50,
          'arcanist',
          []
        );
        expect(effects50.some(e => e.type === 'spell_dc_penalty')).toBe(true);
        expect(effects50.some(e => e.type === 'heat_generation_increase')).toBe(
          true
        );

        // 75% feedback - increased penalties
        const effects75 = HeatFeedbackUtils.calculateFeedbackEffects(
          75,
          'arcanist',
          []
        );
        const attackPenalty = effects75.find(
          e => e.type === 'spell_attack_penalty'
        );
        expect(attackPenalty?.severity).toBe(-2);

        // 90% feedback - spell failure chance
        const effects90 = HeatFeedbackUtils.calculateFeedbackEffects(
          90,
          'arcanist',
          []
        );
        expect(effects90.some(e => e.type === 'spell_failure_chance')).toBe(
          true
        );
        expect(effects90.some(e => e.type === 'casting_time_increase')).toBe(
          true
        );
      });
      ...
      it('should get penalty values from feedback state', () => {
        ...
        mockHeatFeedbackState.feedback.effects =
          HeatFeedbackUtils.calculateFeedbackEffects(50, 'arcanist', []);

        expect(
          HeatFeedbackUtils.getSpellAttackPenalty(mockHeatFeedbackState)
        ).toBe(-1);
        expect(HeatFeedbackUtils.getSpellDcPenalty(mockHeatFeedbackState)).toBe(
          -1
        );
        expect(
          HeatFeedbackUtils.getResourceCostIncrease(mockHeatFeedbackState)
        ).toBe(0); // Not at 75% yet
        expect(
          HeatFeedbackUtils.getSpellFailureChance(mockHeatFeedbackState)
        ).toBe(0); // Not at 90% yet
      });
    });
    ...
    describe('Rest and recovery', () => {
      ...
      it('should apply short rest recovery', () => {
        const rested = HeatFeedbackUtils.applyRest(
          mockHeatFeedbackState,
          'short'
        );
    ...
    describe('Concentration mechanics', () => {
      ...
      it('should start concentration', () => {
        const updated = HeatFeedbackUtils.startConcentration(
          mockHeatFeedbackState,
          concentrationSpell
        );

        expect(updated.concentration.isConcentrating).toBe(true);
        expect(updated.concentration.concentratedSpell).toEqual(
          concentrationSpell
        );
      ...
      it('should end concentration', () => {
        ...
        mockHeatFeedbackState.concentration.concentratedSpell =
          concentrationSpell;

        const updated = HeatFeedbackUtils.endConcentration(
          mockHeatFeedbackState
        );
      ...
      it('should handle concentration saves', () => {
        ...
        expect(
          saveResult.updatedState.concentration.concentrationSavesThisTurn
        ).toBe(1);
    ...
    describe('Validation', () => {
      it('should validate valid heat feedback state', () => {
        const result = HeatFeedbackUtils.validateHeatFeedbackState(
          mockHeatFeedbackState
        );
        expect(result.success).toBe(true);
      });
  ...
  describe('Shared Spellcasting Utilities', () => {
    ...
    describe('Spell component utilities', () => {
      ...
      it('should detect missing costly components', () => {
        ...
        expect(result.missingComponents).toContain(
          'Material component: Diamond worth 100 gp'
        );
      });

      it('should generate component descriptions', () => {
        expect(
          SpellComponentUtils.getComponentDescription(mockComponents)
        ...
        expect(
          SpellComponentUtils.getComponentDescription(simpleComponents)
        ).toBe('V');
      });
    });
