...
describe('Advancement System', () => {
  ...
  describe('Multiclassing Support (Basic)', () => {
    ...
    it('should handle archetype selection at different levels for different classes', () => {
      ...
      expect(
        getAdvancementOptions(1, 'arcanist').archetypeSelection
      ).toBeUndefined();
      expect(
        getAdvancementOptions(2, 'templar').archetypeSelection
      ).toBeUndefined();
    });
  });
  ...
  describe('Edge Cases and Error Handling', () => {
    ...
    it('should validate complex ability score improvements', () => {
      ...
      const result = applyAbilityScoreImprovements(
        currentScores,
        validImprovements
      );
      expect(result.success).toBe(true);
    ...
    it('should handle advancement choices with all optional fields', () => {
      const choices: AdvancementChoices = {
        ...
        hitPointsGained: 5,
        hitPointMethod: 'average',
        abilityScoreImprovements: [{ ability: 'intelligence', improvement: 2 }],
        classFeatures: [],
        spellsLearned: [],
        skillsGained: [],
        proficienciesGained: [],
