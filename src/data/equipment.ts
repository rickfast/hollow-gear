import type { Armor, Equipment, Mod, Shield, Weapon } from "../types";

// ============================================================================
// MELEE WEAPONS
// ============================================================================
export const WEAPONS: Weapon[] = [
    // Simple Melee
    {
        id: "brass-dagger-001",
        name: "Brass Dagger",
        type: "Weapon",
        weaponType: "Melee Simple",
        tier: "Workshop",
        damage: {
            count: 1,
            die: 4,
            damageType: "Piercing",
        },
        properties: ["Light", "Finesse"],
        cost: 2,
        weight: 1,
    },
    {
        id: "cogwrench-001",
        name: "Cogwrench",
        type: "Weapon",
        weaponType: "Melee Simple",
        tier: "Workshop",
        damage: {
            count: 1,
            die: 6,
            damageType: "Bludgeoning",
        },
        properties: ["Versatile"], // 1d8 versatile
        cost: 5,
        weight: 3,
        description: "Versatile (1d8)",
    },

    // Martial Melee
    {
        id: "steam-hammer-001",
        name: "Steam Hammer",
        type: "Weapon",
        weaponType: "Melee Martial",
        tier: "Workshop",
        damage: {
            count: 1,
            die: 10,
            damageType: "Bludgeoning",
        },
        properties: ["Two-Handed"],
        cost: 15,
        weight: 8,
    },
    {
        id: "volt-blade-001",
        name: "Volt Blade",
        type: "Weapon",
        weaponType: "Melee Martial",
        tier: "Workshop",
        damage: {
            count: 1,
            die: 8,
            damageType: "Slashing",
            bonus: 1, // +1 lightning
        },
        properties: ["Finesse", "Powered"],
        cost: 30,
        weight: 3,
        powered: true,
        description: "+1 lightning damage when powered",
    },
    {
        id: "gear-axe-001",
        name: "Gear Axe",
        type: "Weapon",
        weaponType: "Melee Martial",
        tier: "Workshop",
        damage: {
            count: 1,
            die: 12,
            damageType: "Slashing",
        },
        properties: ["Heavy", "Two-Handed"],
        cost: 25,
        weight: 10,
    },
    {
        id: "aether-spear-001",
        name: "Aether Spear",
        type: "Weapon",
        weaponType: "Melee Martial",
        tier: "Workshop",
        damage: {
            count: 1,
            die: 8,
            damageType: "Piercing",
        },
        properties: ["Thrown", "Reach"],
        range: { normal: 20, max: 60 },
        cost: 20,
        weight: 4,
    },
    {
        id: "monofilament-whip-001",
        name: "Monofilament Whip",
        type: "Weapon",
        weaponType: "Melee Martial",
        tier: "Workshop",
        damage: {
            count: 1,
            die: 6,
            damageType: "Slashing",
        },
        properties: ["Finesse", "Reach"],
        cost: 35,
        weight: 2,
    },
    {
        id: "plasma-pike-001",
        name: "Plasma Pike",
        type: "Weapon",
        weaponType: "Melee Martial",
        tier: "Workshop",
        damage: {
            count: 1,
            die: 10,
            damageType: "Piercing",
        },
        properties: ["Reach", "Two-Handed", "Powered"],
        cost: 40,
        weight: 6,
        powered: true,
    },

    // Ranged Weapons - Simple
    {
        id: "slug-pistol-001",
        name: "Slug Pistol",
        type: "Weapon",
        weaponType: "Ranged Simple",
        tier: "Workshop",
        damage: {
            count: 1,
            die: 6,
            damageType: "Piercing",
        },
        properties: ["Light", "Reload"],
        range: { normal: 30, max: 90 },
        ammoType: "Slugs",
        cost: 10,
        weight: 2,

        description: "Reload (6)",
    },
    {
        id: "repeating-crossbow-001",
        name: "Repeating Crossbow",
        type: "Weapon",
        weaponType: "Ranged Simple",
        tier: "Workshop",
        damage: {
            count: 1,
            die: 8,
            damageType: "Piercing",
        },
        properties: ["Reload"],
        range: { normal: 80, max: 320 },
        ammoType: "Bolts",
        cost: 20,
        weight: 5,

        description: "Reload (6)",
    },
    {
        id: "steam-slingshot-001",
        name: "Steam Slingshot",
        type: "Weapon",
        weaponType: "Ranged Simple",
        tier: "Workshop",
        damage: {
            count: 1,
            die: 4,
            damageType: "Bludgeoning",
        },
        properties: ["Light", "Silent"],
        range: { normal: 20, max: 80 },
        ammoType: "Pellets",
        cost: 5,
        weight: 1,
    },

    // Ranged Weapons - Martial
    {
        id: "rivetgun-001",
        name: "Rivetgun",
        type: "Weapon",
        weaponType: "Ranged Martial",
        tier: "Workshop",
        damage: {
            count: 1,
            die: 8,
            damageType: "Piercing",
        },
        properties: ["Reload"],
        range: { normal: 40, max: 120 },
        ammoType: "Rivets",
        cost: 25,
        weight: 4,

        description: "Reload (5), mod-ready",
    },
    {
        id: "steam-rifle-001",
        name: "Steam Rifle",
        type: "Weapon",
        weaponType: "Ranged Martial",
        tier: "Workshop",
        damage: {
            count: 1,
            die: 10,
            damageType: "Piercing",
        },
        properties: ["Two-Handed", "Reload"],
        range: { normal: 80, max: 240 },
        ammoType: "Slugs",
        cost: 35,
        weight: 7,

        description: "Reload (5)",
    },
    {
        id: "aether-carbine-001",
        name: "Aether Carbine",
        type: "Weapon",
        weaponType: "Ranged Martial",
        tier: "Workshop",
        damage: {
            count: 1,
            die: 8,
            damageType: "Force",
        },
        properties: ["Two-Handed", "Powered"],
        range: { normal: 60, max: 180 },
        ammoType: "AetherCells",
        cost: 50,
        weight: 6,
        powered: true,
    },
    {
        id: "volleygun-001",
        name: "Volleygun",
        type: "Weapon",
        weaponType: "Ranged Martial",
        tier: "Workshop",
        damage: {
            count: 2,
            die: 6,
            damageType: "Piercing",
        },
        properties: ["Burst", "Reload"],
        range: { normal: 30, max: 60 },
        ammoType: "Slugs",
        cost: 45,
        weight: 10,

        description: "Burst (15-ft cone), Reload (1)",
    },

    // Heavy Weapons
    {
        id: "aether-cannon-001",
        name: "Aether Cannon",
        type: "Weapon",
        weaponType: "Heavy",
        tier: "Guild",
        damage: {
            count: 2,
            die: 10,
            damageType: "Lightning",
        },
        properties: ["Two-Handed", "Reload", "Powered", "Heavy"],
        range: { normal: 60, max: 150 },
        ammoType: "AetherCore",
        cost: 120,
        weight: 18,
        powered: true,

        description: "Reload (1)",
    },

    // Expanded Arsenal (New Additions)
    // Simple Melee Additions
    {
        id: "steam-gauntlet-001",
        name: "Steam Gauntlet",
        type: "Weapon",
        weaponType: "Melee Simple",
        tier: "Workshop",
        damage: { count: 1, die: 6, damageType: "Bludgeoning" },
        properties: ["Light"],
        cost: 10,
        weight: 2,
        description: "Reinforced glove with steam-piston impact assist.",
    },
    // Martial Melee Additions
    {
        id: "gearblade-001",
        name: "Gearblade",
        type: "Weapon",
        weaponType: "Melee Martial",
        tier: "Workshop",
        damage: { count: 1, die: 6, damageType: "Slashing" },
        properties: ["Light", "Finesse"],
        cost: 8,
        weight: 2,
        description: "Balanced short blade with microgear spine.",
    },
    {
        id: "coil-sabre-001",
        name: "Coil Sabre",
        type: "Weapon",
        weaponType: "Melee Martial",
        tier: "Guild",
        damage: { count: 1, die: 8, damageType: "Slashing", bonus: 1 }, // +1 lightning
        properties: ["Finesse", "Powered"],
        cost: 45,
        weight: 3,
        powered: true,
        description: "+1 lightning damage when powered.",
    },
    {
        id: "pneumatic-lance-001",
        name: "Pneumatic Lance",
        type: "Weapon",
        weaponType: "Melee Martial",
        tier: "Workshop",
        damage: { count: 1, die: 12, damageType: "Piercing" },
        properties: ["Reach", "Two-Handed"],
        cost: 55,
        weight: 8,
        description: "Pressurized thrust mechanism increases penetration.",
    },
    {
        id: "void-iron-mace-001",
        name: "Void Iron Mace",
        type: "Weapon",
        weaponType: "Melee Martial",
        tier: "Guild",
        damage: { count: 1, die: 8, damageType: "Bludgeoning" },
        properties: ["Powered"],
        cost: 60,
        weight: 5,
        powered: true,
        description: "Adds +1d4 psychic damage when powered (GM adjudication).",
    },
    {
        id: "aetherglass-rapier-001",
        name: "Aetherglass Rapier",
        type: "Weapon",
        weaponType: "Melee Martial",
        tier: "Guild",
        damage: { count: 1, die: 8, damageType: "Piercing" },
        properties: ["Light", "Finesse"],
        cost: 35,
        weight: 2, // reduced weight
        description: "Lightweight psionically conductive blade (−25% weight).",
    },
    // Ranged / Heavy Additions
    {
        id: "magnetron-bow-001",
        name: "Magnetron Bow",
        type: "Weapon",
        weaponType: "Ranged Martial",
        tier: "Workshop",
        damage: { count: 1, die: 8, damageType: "Piercing" },
        properties: ["Two-Handed"],
        range: { normal: 90, max: 300 },
        ammoType: "Bolts",
        cost: 40,
        weight: 4,
        description: "Pulls struck metal-wielding target 5 ft (Str save DC 13).",
    },
    {
        id: "aetherglass-repeater-001",
        name: "Aetherglass Repeater",
        type: "Weapon",
        weaponType: "Ranged Martial",
        tier: "Guild",
        damage: { count: 1, die: 6, damageType: "Force" },
        properties: ["Two-Handed", "Reload", "Powered"],
        range: { normal: 50, max: 150 },
        ammoType: "AetherCells",
        cost: 60,
        weight: 4,
        powered: true,
        description: "Reload (8). Lightweight frame of aetherglass panels.",
    },
    {
        id: "arc-thrower-001",
        name: "Arc Thrower",
        type: "Weapon",
        weaponType: "Heavy",
        tier: "Guild",
        damage: { count: 2, die: 8, damageType: "Lightning" },
        properties: ["Two-Handed", "Reload", "Powered", "Heavy"],
        range: { normal: 40, max: 120 },
        ammoType: "AetherCells",
        cost: 90,
        weight: 14,
        powered: true,
        description: "Once/short rest may fire 15-ft cone instead of single target.",
    },
    {
        id: "void-resonator-001",
        name: "Void Resonator",
        type: "Weapon",
        weaponType: "Heavy",
        tier: "Relic",
        damage: { count: 2, die: 10, damageType: "Psychic" },
        properties: ["Two-Handed", "Reload", "Powered", "Heavy"],
        range: { normal: 60, max: 180 },
        ammoType: "AetherCore",
        cost: 150,
        weight: 20,
        powered: true,
        description: "Target damaged must make Con save or suffer fear (GM discretion).",
    },
];

// ============================================================================
// ARMOR
// ============================================================================
export const ARMOR: Armor[] = [
    // Light Armor
    {
        id: "steamweave-vest-001",
        name: "Steamweave Vest",
        type: "Armor",
        armorType: "Light",
        tier: "Workshop",
        armorClass: "11 + Dex",
        stealthDisadvantage: false,
        cost: 10,
        weight: 8,

        description: "Common fabric-metal hybrid.",
    },
    {
        id: "wireweave-jacket-001",
        name: "Wireweave Jacket",
        type: "Armor",
        armorType: "Light",
        tier: "Workshop",
        armorClass: "12 + Dex",
        stealthDisadvantage: false,
        cost: 25,
        weight: 10,

        description: "Resists shock & heat.",
    },
    {
        id: "aetherweave-coat-001",
        name: "Aetherweave Coat",
        type: "Armor",
        armorType: "Light",
        tier: "Guild",
        armorClass: "12 + Dex",
        stealthDisadvantage: false,
        cost: 40,
        weight: 9,

        description: "+1 vs psionics. Adv on Stealth vs machines. Conducts Aether energy safely.",
    },

    // Medium Armor
    {
        id: "gearmail-hauberk-001",
        name: "Gearmail Hauberk",
        type: "Armor",
        armorType: "Medium",
        tier: "Workshop",
        armorClass: "13 + Dex (max 2)",
        stealthDisadvantage: true,
        cost: 50,
        weight: 20,

        description: "Common militia armor.",
    },
    {
        id: "steamjack-coat-001",
        name: "Steamjack Coat",
        type: "Armor",
        armorType: "Medium",
        tier: "Workshop",
        armorClass: "14 + Dex (max 2)",
        stealthDisadvantage: true,
        cost: 75,
        weight: 25,

        description: "Has mod slot for Steam Vent.",
    },
    {
        id: "aetherplate-hauberk-001",
        name: "Aetherplate Hauberk",
        type: "Armor",
        armorType: "Medium",
        tier: "Guild",
        armorClass: "15 + Dex (max 2)",
        stealthDisadvantage: true,
        cost: 100,
        weight: 30,

        description: "Resist psychic dmg 1/long rest.",
    },

    // Heavy Armor
    {
        id: "rivetmail-plate-001",
        name: "Rivetmail Plate",
        type: "Armor",
        armorType: "Heavy",
        tier: "Workshop",
        armorClass: 16,
        strengthRequirement: 13,
        stealthDisadvantage: true,
        cost: 150,
        weight: 45,

        description: "Common industrial plate.",
    },
    {
        id: "cog-knight-armor-001",
        name: "Cog Knight Armor",
        type: "Armor",
        armorType: "Heavy",
        tier: "Guild",
        armorClass: 17,
        strengthRequirement: 13,
        stealthDisadvantage: true,
        cost: 250,
        weight: 50,

        description: "Templar make; +1 vs radiant.",
    },
    {
        id: "powered-exo-suit-001",
        name: "Powered Exo-Suit",
        type: "Armor",
        armorType: "Heavy",
        tier: "Relic",
        armorClass: "18 + Str (max +2)",
        strengthRequirement: 15,
        stealthDisadvantage: true,
        powered: true,
        cost: 500,
        weight: 65,

        description:
            "Steam-driven servo plate; requires maintenance (1 Cog/day). Consumes 1 Aether Cell per hour.",
    },
    // Expanded Armor Selection
    // Light Armor Additions
    {
        id: "zephyr-mesh-001",
        name: "Zephyr Mesh",
        type: "Armor",
        armorType: "Light",
        tier: "Guild",
        armorClass: "13 + Dex",
        stealthDisadvantage: false,
        cost: 35,
        weight: 7,
        description: "Reduces Heat gain from extended movement (GM: -1 per hour travel).",
    },
    {
        id: "voidweave-shroud-001",
        name: "Voidweave Shroud",
        type: "Armor",
        armorType: "Light",
        tier: "Guild",
        armorClass: "13 + Dex",
        stealthDisadvantage: false,
        cost: 55,
        weight: 8,
        description: "Advantage on saves vs fear; psionic void filaments.",
    },
    // Medium Armor Additions
    {
        id: "magnetron-brigandine-001",
        name: "Magnetron Brigandine",
        type: "Armor",
        armorType: "Medium",
        tier: "Guild",
        armorClass: "15 + Dex (max 2)",
        stealthDisadvantage: true,
        cost: 110,
        weight: 28,
        description: "Bonus action pull metallic object or weapon within 10 ft (1/short rest).",
    },
    {
        id: "steam-linked-harness-001",
        name: "Steam-Linked Harness",
        type: "Armor",
        armorType: "Medium",
        tier: "Guild",
        armorClass: "16 + Dex (max 2)",
        stealthDisadvantage: true,
        cost: 160,
        weight: 32,
        description: "Ignore first level of Heat Stress each day.",
    },
    // Heavy Armor Additions
    {
        id: "living-steel-carapace-001",
        name: "Living Steel Carapace",
        type: "Armor",
        armorType: "Heavy",
        tier: "Relic",
        armorClass: 18,
        strengthRequirement: 15,
        stealthDisadvantage: true,
        powered: true,
        cost: 600,
        weight: 55,
        description: "Self-repairs 2 HP per hour (up to half). Counts as powered.",
    },
    {
        id: "void-iron-bastion-001",
        name: "Void Iron Bastion",
        type: "Armor",
        armorType: "Heavy",
        tier: "Relic",
        armorClass: 19,
        strengthRequirement: 15,
        stealthDisadvantage: true,
        cost: 900,
        weight: 60,
        description: "+1d4 psychic retaliation vs melee (GM adjudication).",
    },
];

// ============================================================================
// SHIELDS
// ============================================================================
export const SHIELDS: Shield[] = [
    {
        id: "standard-shield-001",
        name: "Standard Shield",
        type: "Shield",
        tier: "Workshop",
        armorClassBonus: 2,
        cost: 10,
        weight: 6,
        description: "Basic steel buckler.",
    },
    {
        id: "steam-shield-001",
        name: "Steam Shield",
        type: "Shield",
        tier: "Workshop",
        armorClassBonus: 2,
        cost: 35,
        weight: 10,

        description: "Can release steam cloud (1/short rest).",
    },
    {
        id: "resonant-aegis-001",
        name: "Resonant Aegis",
        type: "Shield",
        tier: "Guild",
        armorClassBonus: 3,
        cost: 60,
        weight: 8,

        description: "+3 vs psionics. Glows when detecting Aether fields.",
    },
    {
        id: "voltaic-shield-001",
        name: "Voltaic Shield",
        type: "Shield",
        tier: "Guild",
        armorClassBonus: 2,
        cost: 70,
        weight: 12,

        description: "Retaliates 1d4 lightning on melee hits.",
    },
    {
        id: "living-steel-shield-001",
        name: "Living Steel Shield",
        type: "Shield",
        tier: "Relic",
        armorClassBonus: 2,
        cost: 700,
        weight: 6,

        description: "Restores 1 HP each hour, up to half total HP.",
    },
];

// ============================================================================
// BASIC EQUIPMENT & ADVENTURING GEAR
// ============================================================================
export const BASIC_EQUIPMENT: Equipment[] = [
    {
        id: "tinkers-tools-001",
        name: "Tinker's Tools",
        type: "Tool",
        tier: "Workshop",
        cost: 25,
        weight: 10,

        description: "Essential for mod crafting and repairs.",
    },
    {
        id: "mechanists-satchel-001",
        name: "Mechanist's Satchel",
        type: "Tool",
        tier: "Workshop",
        cost: 15,
        weight: 6,

        description: "Contains wrenches, microgears, soldering kit.",
    },
    {
        id: "steam-lantern-001",
        name: "Steam Lantern",
        type: "Item",
        tier: "Workshop",
        cost: 10,
        weight: 3,

        description: "Emits bright light 30 ft (requires Aether Cell, 10 hours).",
    },
    {
        id: "aether-compass-001",
        name: "Aether Compass",
        type: "Item",
        tier: "Guild",
        cost: 50,
        weight: 2,

        description: "Always points toward nearest psionic field or powered device.",
    },
    {
        id: "wireweave-rope-50-ft-001",
        name: "Wireweave Rope (50 ft)",
        type: "Item",
        tier: "Workshop",
        cost: 5,
        weight: 5,

        description: "Conductive, can deliver shocks or carry Aether charge.",
    },
    {
        id: "coolant-flask-001",
        name: "Coolant Flask",
        type: "Consumable",
        tier: "Workshop",
        cost: 8,
        weight: 2,

        description: "Reduces Heat by 1d4 when consumed or poured on armor.",
    },
    {
        id: "pressure-tent-001",
        name: "Pressure Tent",
        type: "Item",
        tier: "Workshop",
        cost: 10,
        weight: 8,

        description: "Portable, steam-insulated shelter.",
    },
    {
        id: "repair-paste-vial-001",
        name: "Repair Paste (vial)",
        type: "Consumable",
        tier: "Workshop",
        cost: 15,
        weight: 0.5,

        description: "Repairs 1d8 HP of damage to armor, weapons, or constructs.",
    },
    {
        id: "aether-dust-vial-001",
        name: "Aether Dust Vial",
        type: "Consumable",
        tier: "Workshop",
        cost: 25,
        weight: 0.5,

        description: "Refined psionic residue, reagent for mod crafting.",
    },
    {
        id: "chronometer-cog-001",
        name: "Chronometer Cog",
        type: "Item",
        tier: "Workshop",
        cost: 5,
        weight: 0,

        description: "Keeps precise time for 100 years; valued by scholars.",
    },
    {
        id: "aether-lamp-001",
        name: "Aether Lamp",
        type: "Item",
        tier: "Workshop",
        cost: 15,
        weight: 3,

        description: "30-ft blue light, 10-hour Aether Cell.",
    },
    {
        id: "steam-vent-harness-001",
        name: "Steam Vent Harness",
        type: "Item",
        tier: "Workshop",
        cost: 40,
        weight: 10,

        description: "Emit steam to obscure area (1/short rest).",
    },
    {
        id: "goggles-of-clarity-001",
        name: "Goggles of Clarity",
        type: "Item",
        tier: "Workshop",
        cost: 25,
        weight: 0,

        description: "Immune to blindness from steam or flash.",
    },
];

// ============================================================================
// ARCANE ITEMS
// ============================================================================
export const ARCANE_ITEMS: Equipment[] = [
    // Common Arcane Items
    {
        id: "aether-cell-001",
        name: "Aether Cell",
        type: "AetherCell",
        tier: "Workshop",
        cost: 10,
        weight: 0.5,

        description: "Rechargeable psionic battery used by weapons and armor. 10 charges.",
    },
    {
        id: "steam-vent-harness-002",
        name: "Steam Vent Harness",
        type: "Item",
        tier: "Workshop",
        cost: 40,
        weight: 10,

        description: "Emits a 10-ft radius steam cloud (1/short rest); clears 2 Heat.",
    },
    {
        id: "goggles-of-clarity-002",
        name: "Goggles of Clarity",
        type: "Item",
        tier: "Workshop",
        cost: 25,
        weight: 0,

        description: "Immune to blindness from steam, sparks, or psionic flashes.",
    },
    {
        id: "aether-lamp-002",
        name: "Aether Lamp",
        type: "Item",
        tier: "Workshop",
        cost: 15,
        weight: 3,

        description: "Emits pale blue light; reveals psionic signatures within 30 ft.",
    },

    // Uncommon Arcane Items
    {
        id: "cloak-of-conductivity-001",
        name: "Cloak of Conductivity",
        type: "Item",
        tier: "Guild",
        cost: 100,
        weight: 3,

        description: "Gain resistance to lightning damage. Wearing metal armor negates this.",
    },
    {
        id: "brass-familiar-001",
        name: "Brass Familiar",
        type: "Item",
        tier: "Guild",
        cost: 150,
        weight: 1,

        description: "Tiny automaton (AC 13, 5 HP) that acts as a familiar once per day.",
    },
    {
        id: "magnetron-gauntlet-001",
        name: "Magnetron Gauntlet",
        type: "Item",
        tier: "Guild",
        cost: 125,
        weight: 2,

        description: "Pull a metal object or weapon up to 30 ft once per short rest.",
    },
    {
        id: "cogsmiths-ring-001",
        name: "Cogsmith's Ring",
        type: "Item",
        tier: "Guild",
        cost: 80,
        weight: 0,

        description: "While wearing this, gain +1 on Tinkering and Arcana checks.",
    },
    {
        id: "mnemonic-lens-001",
        name: "Mnemonic Lens",
        type: "Item",
        tier: "Guild",
        cost: 175,
        weight: 0,

        description:
            "Once per long rest, recall a scene you witnessed within the last week with perfect clarity.",
    },
    {
        id: "wand-of-residual-charge-001",
        name: "Wand of Residual Charge",
        type: "Focus",
        tier: "Guild",
        cost: 200,
        weight: 1,

        description:
            "Expend a charge to deal 1d8 lightning or force damage (6 charges, recharges 1d4/day).",
    },

    // Rare Arcane Items
    {
        id: "aether-spire-shard-001",
        name: "Aether Spire Shard",
        type: "Focus",
        tier: "Relic",
        cost: 500,
        weight: 1,

        description: "Crystal focus. Gain +1 to psionic save DCs and power attacks.",
    },
    {
        id: "steamheart-core-001",
        name: "Steamheart Core",
        type: "Item",
        tier: "Relic",
        cost: 450,
        weight: 3,

        description: "Replace a weapon's power source; doubles Aether Cell capacity.",
    },
    {
        id: "voltaic-cloak-001",
        name: "Voltaic Cloak",
        type: "Item",
        tier: "Relic",
        cost: 400,
        weight: 4,

        description: "When hit in melee, retaliate for 1d4 lightning damage.",
    },
    {
        id: "chrono-regulator-001",
        name: "Chrono Regulator",
        type: "Item",
        tier: "Relic",
        cost: 800,
        weight: 2,

        description: "Once per long rest, rewind your last action by 6 seconds (GM discretion).",
    },
    {
        id: "resonant-helm-001",
        name: "Resonant Helm",
        type: "Item",
        tier: "Relic",
        cost: 600,
        weight: 3,

        description: "Grants advantage on saves vs. psionics; wearer emits soft harmonic hum.",
    },
    {
        id: "soul-mirror-pendant-001",
        name: "Soul Mirror Pendant",
        type: "Item",
        tier: "Relic",
        cost: 550,
        weight: 0,

        description: "Absorbs up to 10 psychic damage/day; release it as 1d10 healing.",
    },
    {
        id: "aether-linked-gloves-001",
        name: "Aether-Linked Gloves",
        type: "Item",
        tier: "Relic",
        cost: 650,
        weight: 1,

        description: "When wielding powered weapons, add +1 to attack and damage rolls.",
    },
];

// ============================================================================
// MODS - Equipment Modifications (Chapter 5)
// ============================================================================
export const MODS: Mod[] = [
    // ========================================================================
    // WEAPON MODS
    // ========================================================================
    {
        id: "overclock-coil-001",
        name: "Overclock Coil",
        tier: "I - Common",
        modType: "Power",
        effect: "+1d8 damage (once/short rest)",
        craftDC: 10,
        craftTime: 2,
        cost: 25,
        malfunctionChance: 5,
        notes: "Overheat risk. 5% malfunction chance.",
    },
    {
        id: "steam-vent-array-001",
        name: "Steam Vent Array",
        tier: "I - Common",
        modType: "Utility",
        effect: "Create 10-ft obscuring cloud for 1 round (1/short rest)",
        craftDC: 10,
        craftTime: 2,
        cost: 25,
        notes: "Also cools armor.",
    },
    {
        id: "voltaic-mesh-001",
        name: "Voltaic Mesh",
        tier: "II - Advanced",
        modType: "Reactive",
        effect: "Deals 1d4 lightning to melee attackers",
        craftDC: 13,
        craftTime: 4,
        cost: 75,
        notes: "Counts as powered.",
    },
    {
        id: "resonant-core-001",
        name: "Resonant Core",
        tier: "II - Advanced",
        modType: "Psionic",
        effect: "Converts damage to psychic (once/turn)",
        craftDC: 13,
        craftTime: 4,
        cost: 75,
        notes: "Requires psionic focus.",
    },
    {
        id: "magnetron-housing-001",
        name: "Magnetron Housing",
        tier: "II - Advanced",
        modType: "Utility",
        effect: "You can disarm metallic weapons within 5 ft as bonus action",
        craftDC: 13,
        craftTime: 4,
        cost: 75,
        notes: "Strength save DC 13.",
    },
    {
        id: "aether-injector-001",
        name: "Aether Injector",
        tier: "III - Relic",
        modType: "Power",
        effect: "Spend 1 Aether Cell to double weapon damage dice for 1 round",
        craftDC: 16,
        craftTime: 8,
        cost: 200,
        malfunctionChance: 10,
        notes: "High malfunction risk.",
    },
    {
        id: "cryo-edge-001",
        name: "Cryo Edge",
        tier: "III - Relic",
        modType: "Elemental",
        effect: "On hit: target's speed reduced by 10 ft until end of next turn",
        craftDC: 16,
        craftTime: 8,
        cost: 200,
    },
    {
        id: "entropy-lattice-001",
        name: "Entropy Lattice",
        tier: "IV - Prototype",
        modType: "Psionic",
        effect: "Adds +1d6 force damage and 5-ft knockback",
        craftDC: 18,
        craftTime: 16,
        cost: 400,
        notes: "Sentient; whispers in combat.",
    },
    {
        id: "soul-capacitor-001",
        name: "Soul Capacitor",
        tier: "IV - Prototype",
        modType: "Power",
        effect: "Stores 10 psychic dmg; next hit adds it",
        craftDC: 18,
        craftTime: 16,
        cost: 400,
        notes: "Emits eerie hum when charged.",
    },

    // ========================================================================
    // ARMOR MODS
    // ========================================================================
    {
        id: "pressure-vents-001",
        name: "Pressure Vents",
        tier: "I - Common",
        modType: "Utility",
        effect: "Release steam to remove Heat Stress",
        craftDC: 10,
        craftTime: 2,
        cost: 25,
        notes: "1/short rest.",
    },
    {
        id: "reinforced-plating-001",
        name: "Reinforced Plating",
        tier: "I - Common",
        modType: "Defense",
        effect: "+1 AC vs slashing or piercing",
        craftDC: 10,
        craftTime: 2,
        cost: 25,
        notes: "Stackable up to +2.",
    },
    {
        id: "reflective-sigil-001",
        name: "Reflective Sigil",
        tier: "II - Advanced",
        modType: "Reactive",
        effect: "Reflects 1 spell or psionic effect (1/long rest)",
        craftDC: 13,
        craftTime: 4,
        cost: 75,
        notes: "Consumes sigil charge.",
    },
    {
        id: "flux-dampener-001",
        name: "Flux Dampener",
        tier: "II - Advanced",
        modType: "Psionic",
        effect: "Reduces psychic damage by 3",
        craftDC: 13,
        craftTime: 4,
        cost: 75,
        notes: "Passive.",
    },
    {
        id: "servo-stabilizers-001",
        name: "Servo Stabilizers",
        tier: "II - Advanced",
        modType: "Power",
        effect: "+5 ft movement; ignore heavy armor Strength req",
        craftDC: 13,
        craftTime: 4,
        cost: 75,
        notes: "Requires maintenance.",
    },
    {
        id: "overheat-regulator-001",
        name: "Overheat Regulator",
        tier: "III - Relic",
        modType: "Utility",
        effect: "Ignore 1 level of Heat Stress per short rest",
        craftDC: 16,
        craftTime: 8,
        cost: 200,
        notes: "Cannot stack.",
    },
    {
        id: "aether-amplifier-001",
        name: "Aether Amplifier",
        tier: "III - Relic",
        modType: "Psionic",
        effect: "Gain +1 to attack rolls or save DCs for psionic powers",
        craftDC: 16,
        craftTime: 8,
        cost: 200,
    },
    {
        id: "graviton-anchor-001",
        name: "Graviton Anchor",
        tier: "IV - Prototype",
        modType: "Power",
        effect: "Advantage vs knockback, forced move, or push/pull",
        craftDC: 18,
        craftTime: 16,
        cost: 400,
        notes: "Creates heavy hum field.",
    },
    {
        id: "soul-mirror-001",
        name: "Soul Mirror",
        tier: "IV - Prototype",
        modType: "Psionic",
        effect: "Transfer ½ psychic dmg taken to nearest enemy within 10 ft",
        craftDC: 18,
        craftTime: 16,
        cost: 400,
        notes: "Once per short rest.",
    },

    // ========================================================================
    // SHIELD MODS
    // ========================================================================
    {
        id: "steam-deflector-001",
        name: "Steam Deflector",
        tier: "I - Common",
        modType: "Defense",
        effect: "Gain +2 AC vs ranged attacks for 1 round (1/short rest)",
        craftDC: 10,
        craftTime: 2,
        cost: 25,
        notes: "Vent-assisted.",
    },
    {
        id: "resonant-aegis-core-001",
        name: "Resonant Aegis Core",
        tier: "II - Advanced",
        modType: "Defense",
        effect: "Allies within 10 ft gain +1 AC",
        craftDC: 13,
        craftTime: 4,
        cost: 75,
        notes: "Emits low harmonic field.",
    },
    {
        id: "aether-ward-lens-001",
        name: "Aether Ward Lens",
        tier: "III - Relic",
        modType: "Psionic",
        effect: "Absorbs 10 psychic dmg per day",
        craftDC: 16,
        craftTime: 8,
        cost: 200,
        notes: "Refills at dawn.",
    },
    {
        id: "voltaic-pulse-coil-001",
        name: "Voltaic Pulse Coil",
        tier: "III - Relic",
        modType: "Reactive",
        effect: "When hit in melee, emit 5-ft burst (1d6 lightning)",
        craftDC: 16,
        craftTime: 8,
        cost: 200,
    },
    {
        id: "kinetic-redirector-001",
        name: "Kinetic Redirector",
        tier: "IV - Prototype",
        modType: "Reactive",
        effect: "Once/long rest, redirect melee attack to another target",
        craftDC: 18,
        craftTime: 16,
        cost: 400,
        notes: "Reaction.",
    },
];

export const ALL_EQUIPMENT: Equipment[] = [
    ...WEAPONS,
    ...ARMOR,
    ...SHIELDS,
    ...BASIC_EQUIPMENT,
    ...ARCANE_ITEMS,
];

export const EQUIPMENT_BY_ID: Record<string, Equipment> = ALL_EQUIPMENT.reduce(
    (acc, item) => {
        acc[item.id] = item;
        return acc;
    },
    {} as Record<string, Equipment>
);
