 ...
 */
export type AetherCellCondition = 'pristine' | 'good' | 'worn' | 'damaged' | 'depleted';
 ...
 */
export const AETHER_CELL_SPECS: Record<AetherCellType, {
  maxCharges: number;
 ...
 */
export const CONDITION_EFFECTS: Record<AetherCellCondition, {
 ...
 */
export function convertToTotalCogs(currency: Pick<CurrencyData, 'cogs' | 'gears' | 'cores'>): number {
  return currency.cogs + 
 ...
 */
export function convertFromTotalCogs(totalCogs: number): Pick<CurrencyData, 'cogs' | 'gears' | 'cores'> {
  ...
  const gears = Math.floor(remainingAfterCores / CURRENCY_CONVERSION.COGS_PER_GEAR);
...
export function createAetherCell(
  ...
  return {
    id: id || `aether-cell-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
    type,
...
export function rechargeAetherCell(
  ...
  const effectiveCharges = Math.floor(chargesToAdd * conditionEffect.chargeEfficiency);
...
export function calculateAetherCellValue(cell: AetherCell): { gears: number } {
  ...
  const conditionMultiplier = CONDITION_EFFECTS[cell.condition].chargeEfficiency;
  const chargeRatio = cell.charges / cell.maxCharges;
  
  const adjustedValue = Math.floor(baseValue * conditionMultiplier * chargeRatio);
