import { DroneArchetypeSelector } from "@/components/drone-archetype-selector";
...
type DroneBuilderStep = "basics" | "type" | "archetype" | "owner" | "review";
        ...
        try {
            updateCharacter(deletedDrone.ownerId, (vm) => {
                ...
                return {
                    ...
                    activeDroneId:
                        character.activeDroneId === droneId ? undefined : character.activeDroneId,
                };
            });
        } catch (error) {
...
export function DroneBuilderPage() {
    ...
    useEffect(() => {
        if (isEditMode && droneId) {
            ...
            setTimeout(() => {
                ...
                setTemplateId(drone.templateId);
                setArchetypeId(drone.archetypeId || "");
    ...
    const validateCurrentStep = (): boolean => {
        ...
        switch (step) {
            case "basics":
                ...
                if (!name.trim()) {
                    ...
                    const isDuplicate = allDrones.some(
                        (d) =>
                            d.name.toLowerCase() === name.trim().toLowerCase() && d.id !== droneId
                    );
                    if (isDuplicate) {
            ...
            case "type":
                if (!templateId) {
                    errors.templateId = "Please select a drone template";
                }
                break;
    ...
    const handleCreate = async () => {
        try {
            ...
            const allDrones = droneStorageService.getAllDrones();
            const isDuplicate = allDrones.some(
                (d) => d.name.toLowerCase() === name.trim().toLowerCase() && d.id !== droneId
            );
            if (isDuplicate) {
                setFieldErrors({
                    name: `A drone named "${name}" already exists. Please choose a unique name.`,
                });
                setError("Please fix the errors before continuing");
                setStep("basics");
                setIsSaving(false);
            ...
            if (isEditMode && existingDrone) {
                ...
                if (ownerId && ownerId !== previousOwnerId && !canAddDrone) {
                    setFieldErrors({
                        owner: `${selectedOwner?.name} has reached their drone limit (${currentDroneCount}/${droneLimit})`,
                    });
                    setError(
                        `Cannot assign drone: ${selectedOwner?.name} has reached their drone limit (${currentDroneCount}/${droneLimit})`
                    );
                    setIsSaving(false);
                    window.scrollTo({ top: 0, behavior: "smooth" });
                    return;
                ...
                if (!template) {
                    setFieldErrors({ templateId: "Invalid drone template selected" });
                    setError("Invalid drone template selected");
                    setIsSaving(false);
                    window.scrollTo({ top: 0, behavior: "smooth" });
                    return;
                }
                    ...
                    if (previousOwnerId) {
                        updateCharacter(previousOwnerId, (vm) => {
                            ...
                            return {
                                ...
                                drones: updatedDrones,
                                activeDroneId:
                                    character.activeDroneId === existingDrone.id
                                        ? undefined
                ...
                if (ownerId && !canAddDrone) {
                    ...
                    setError(
                        `Cannot add drone: ${selectedOwner?.name} has reached their drone limit (${currentDroneCount}/${droneLimit})`
                    );
                    setIsSaving(false);
                    window.scrollTo({ top: 0, behavior: "smooth" });
                    return;
                ...
                builder.setName(name).setTemplateId(templateId).setLevel(1);
                ...
                if (archetypeId) {
                    builder.setArchetype(archetypeId);
                }
            ...
            const errorMessage =
                err instanceof Error
                    ? err.message
                    : `Failed to ${isEditMode ? "update" : "create"} drone`;
    ...
    // Get template for current template ID
    const selectedTemplate = templateId ? DRONE_TEMPLATES_BY_ID[templateId] : null;

    // Get all characters for owner selection
    const allCharacters = getAllCharacters();

    // Filter to Artifex characters only
    const artifexCharacters = allCharacters.filter(
        (vm) => vm.summary.class === "Artifex" || vm.summary.fullClass.includes("Artifex")
    );
                                ...
                                >
                                    {steps.map((s, idx) => (
                                        <Chip
                                            ...
                                            aria-current={
                                                idx === currentStepIndex ? "step" : undefined
                                            }
                                        >
                                            {idx + 1}. {s.label}
                                        </Chip>
                            ...
                            >
                                ...
                                {step === "basics" && (
                                    ...
                                    >
                                        ...
                                        <Input
                                            ...
                                            description={
                                                !fieldErrors.name
                                                    ? "Give your drone a unique name (1-50 characters)"
                                                    : undefined
                                            }
                                        ...
                                        >
                                            💡 <strong>Tip:</strong> Choose a name that fits your
                                            mechanical companion in the steampunk world of Hollow
                                            Gear. Names like "Cogsworth", "Steamwhisker", or
                                            "Brassbeard" work well!
                                        </div>
                                    </div>
                                )}
                                ...
                                {step === "owner" && (
                                    ...
                                    >
                                        ...
                                        >
                                            💡 <strong>Tip:</strong> Optionally assign this drone to
                                            an Artifex character. Each Artifex can only have one
                                            active drone at a time. You can skip this step and
                                        ...
                                        {artifexCharacters.length > 0 ? (
                                            <>
                                                ...
                                                >
                                                    {artifexCharacters.map((vm) => {
                                                        const droneCount =
                                                            vm.toCharacter().drones?.length || 0;
                                                        const atLimit =
                                                            droneCount >= droneLimit &&
                                                            vm.summary.id !== ownerId;
                                                        return (
                                                            <SelectItem
                                                                key={vm.summary.id}
                                                                isDisabled={atLimit}
                                                            >
                                                                {vm.summary.name} (Level{" "}
                                                                {vm.summary.level})
                                                                {atLimit &&
                                                                    " - Drone limit reached"}
                                                            </SelectItem>
                                                ...
                                                {ownerId && !canAddDrone && !fieldErrors.owner && (
                                                    ...
                                                    >
                                                        <CardBody>
                                                            ...
                                                            >
                                                                ⚠️ This character has reached their
                                                                drone limit ({currentDroneCount}/
                                                                {droneLimit}). Each Artifex can only
                                                                have one active drone at a time.
                                            ...
                                            <Card>
                                                <CardBody>
                                                    ...
                                                    >
                                                        No Artifex characters found. Create an
                                                        Artifex character first to assign this drone
                                                        to them.
                                                    </p>
                                                </CardBody>
                                            </Card>
                                ...
                                {step === "review" && (
                                    ...
                                    >
                                                ...
                                                >
                                                    ...
                                                    {archetypeId && (
                                                        <div>
                                                            ...
                                                            {DRONE_ARCHETYPES_BY_ID[archetypeId]
                                                                ?.name || archetypeId}
                                                        </div>
                                                    )}
                                                    <div>
                                        ...
                                        >
                                            ✅ <strong>Ready to go!</strong> Review your drone
                                            configuration above. Click "
                                            {isEditMode ? "Save Changes" : "Create Drone"}" to
                                            finalize your mechanical companion.
                                        </div>
                                    </div>
                                )}
                            </div>
                            ...
                            >
                                ...
                                {step !== "review" ? (
                                    ...
                                    <Button
                                        ...
                                        isDisabled={isSaving}
                                        aria-label={
                                            isEditMode ? "Save drone changes" : "Create new drone"
                                        }
                                    >
                                        {isSaving
                                            ? "Saving..."
